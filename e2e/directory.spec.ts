...
test('the directory lists every schema on the relay, hides on request, and subscribing makes the feed at /landing/ yours', async ({ page }) => {
  ...
  // Signed out, the feed at /landing/ is the directory's: the newest lists and the hottest posts across them.
  await page.goto('/landing/')
  await expect(page.getByRole('heading', { name: 'Across the newest lists' })).toBeVisible()
  await expect(page.locator('article').first()).toBeVisible()

  // Subscribe — privately, through the extension's NIP-44 — and the feed becomes yours.
  ...
  // Unsubscribe from the list's page, and the feed is the directory's again.
  await page.goto(`/r/${nip19.npubEncode(CURATOR)}/${IDENTIFIER}/`)
