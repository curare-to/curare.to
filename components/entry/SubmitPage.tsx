...
export function SubmitPage() {
  ...
  return <ResolvedSubmit list={list} edit={edit} />
}

function ResolvedSubmit({ list, edit }: { list: ListRef; edit: string | null }) {
  ...
  return <Form schema={resolved.schema} list={list} edit={edit} />
}

function Form({ schema, list, edit }: { schema: CuratedSchema; list: ListRef; edit: string | null }) {
  const snapshot = useList(schema)
  const session = useSession()
  const [published, setPublished] = useState<Published | null>(null)

  useEffect(() => {
    document.title = `${edit ? 'Edit' : 'Suggest'} · ${listDisplayName(schema, list)} · curare.to`
  }, [schema, list, edit])
    ...
    return (
      <div className="mx-auto max-w-2xl px-4 py-10">
        ...
        <p className="mt-2 text-ink-2">
          {published.accepted} of {published.total} relays accepted it. It is in the list&apos;s queue now
          {schema.namespace ? ' — the curator decides whether it reaches the front page' : ''}.
        </p>
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8">
      ...
      <h1 className="mt-2 text-2xl font-semibold tracking-tight">
        {editing ? 'Edit your suggestion' : `Suggest to ${listDisplayName(schema, list)}`}
      </h1>
      ...
      <EntryForm key={editing?.id ?? 'new'} schema={schema} editing={editing} groups={snapshot.groups} onPublished={setPublished} />
    </div>
  )
}
